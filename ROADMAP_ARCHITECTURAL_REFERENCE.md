...
- **U11. ~~Risk Subtype System~~** ✅ _(v2.11.0)_: Added schema-driven subtype system with 9 built-in subtypes under `entities.risk.subtypes`. Each subtype defines `applies_to` levels and optional `extension_fields` (string, enum, boolean, integer, float). Extension fields stored as flat `ext_*` properties on `:Risk` nodes. Full stack: YAML → schema_loader → database queries → UI form/list → statistics dashboard → Excel import/export. Zero impact on exposure engine.
