...
class LayoutManager:
    """Gestionnaire de sauvegarde/restauration des positions des nœuds"""
    ...
    def _load_layouts(self):
        """Charge les layouts depuis le fichier JSON"""
    ...
    def _save_layouts(self):
        """Sauvegarde les layouts dans le fichier JSON"""
    ...
    def list_layouts(self):
        """Liste tous les layouts sauvegardés avec leurs métadonnées"""
        return {
            name: {
...
def generate_layered_layout(nodes: list) -> dict:
    """Génère un layout en couches (Strategic en haut, Operational en bas)"""
...
def generate_category_layout(nodes: list) -> dict:
    """Génère un layout groupé par catégories en grille 2x2"""
    categories = ["Programme", "Produit", "Industriel", "Supply Chain"]
    positions = {}
    
    # Grille 2x2
    grid_positions = [
        (200, 200),   # Programme (haut gauche)
        (600, 200),   # Produit (haut droit)
        (200, 500),   # Industriel (bas gauche)
        (600, 500)    # Supply Chain (bas droit)
    ]
...
def render_graph(nodes: list, edges: list, color_by: str = "level", physics_enabled: bool = True, positions: dict = None):
    """Génère et affiche le graphe interactif avec PyVis (avec positions optionnelles)"""
    if not nodes:
    ...
    for node in nodes:
        ...
        # Si des positions sont fournies, les appliquer et fixer le nœud
...
def main():
    ...
    with tab_viz:
        ...
        with col_filters:
            ...
            st.markdown("---")
            st.markdown("### 💾 Gestion des layouts")
            
            # Initialiser le layout manager
            ...
            with st.expander("💾 Sauvegarder le layout actuel"):
                ...
                with col_save_1:
                    if st.button("💾 Sauvegarder (manuel)", key="save_manual", use_container_width=True):
                        st.warning("⚠️ Fonctionnalité en cours de développement.\n\nPour Phase 1, utilisez les layouts prédéfinis ci-dessous.")
                
                with col_save_2:
                    if st.button("🎨 Layout en couches", key="save_layered", use_container_width=True):
            ...
            with st.expander("📂 Charger un layout"):
                ...
                if saved_layouts:
                    ...
                    selected_layout = st.selectbox(
                        ...
                        format_func=lambda x: f"{x} ({saved_layouts[x]['node_count']} nœuds)",
                        key="select_layout"
                    )
                    ...
                    with col_load_1:
                        if st.button("📂 Charger", key="load_layout", use_container_width=True):
                            st.session_state.selected_layout_name = selected_layout
                            st.success(f"✅ Layout '{selected_layout}' chargé !")
                            st.rerun()
                    
                    with col_load_2:
                        if st.button("🗑️ Supprimer", key="delete_layout", use_container_width=True):
                            ...
                            st.success(f"✅ Layout '{selected_layout}' supprimé !")
                            st.rerun()
                else:
                    st.info("Aucun layout sauvegardé. Créez-en un ci-dessus !")
            
            # Section layouts prédéfinis
            with st.expander("🎨 Layouts prédéfinis"):
                col_preset_1, col_preset_2 = st.columns(2)
                
                with col_preset_1:
                    if st.button("📊 En couches", key="preset_layered", use_container_width=True, help="Strategic en haut, Operational en bas"):
                        nodes, _ = manager.get_graph_data(None)
                        positions = generate_layered_layout(nodes)
                        auto_name = f"couches_{datetime.now().strftime('%Y%m%d_%H%M')}"
                        layout_mgr.save_layout(auto_name, positions)
                        st.session_state.selected_layout_name = auto_name
                        st.success("✅ Layout en couches appliqué !")
                        st.rerun()
                
                with col_preset_2:
                    if st.button("🗂️ Par catégories", key="preset_categories", use_container_width=True, help="Groupement par catégories en grille"):
                        nodes, _ = manager.get_graph_data(None)
                        positions = generate_category_layout(nodes)
                        auto_name = f"categories_{datetime.now().strftime('%Y%m%d_%H%M')}"
                        layout_mgr.save_layout(auto_name, positions)
                        st.session_state.selected_layout_name = auto_name
                        st.success("✅ Layout par catégories appliqué !")
                        st.rerun()
                
                if st.button("🔄 Réinitialiser (auto)", key="reset_layout", use_container_width=True, help="Retour à l'organisation automatique"):
                    if "selected_layout_name" in st.session_state:
                        del st.session_state.selected_layout_name
                    st.success("✅ Layout réinitialisé !")
                    st.rerun()
